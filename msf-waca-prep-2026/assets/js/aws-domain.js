...
(function(){
  ...
  let plan={}; try{plan=JSON.parse(localStorage.getItem('msfWacaPrep2026Plan')||'{}')||{}}catch(e){}
  ...
  days.forEach(d=>d.querySelector('.sprint-check')?.addEventListener('click',()=>{plan[d.dataset.day]=!plan[d.dataset.day];localStorage.setItem('msfWacaPrep2026Plan',JSON.stringify(plan));syncPlan()}));syncPlan();
  const qs=window.MSF_QUESTIONS||[]; if(!qs.length)return;
  let st={};try{st=JSON.parse(localStorage.getItem('msfWacaPrep2026QuestionState')||'{}')||{}}catch(e){}
  ...
  const queue=document.getElementById('smartQueue'); if(queue)queue.innerHTML=rows.map(q=>`<a class="review-item" href="/msf-waca-prep-2026/question-bank.html#q-${q.id}"><span class="rank">#${q.interview_rank}</span><span><b>${q.question_en}</b><small>${q.study_track||q.category_en}</small></span><span>→</span></a>`).join('')||'<div class="callout success"><b>No reviews due.</b> Run a mock or open P1.</div>';
