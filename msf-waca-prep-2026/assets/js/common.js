(function(){
  ...
  const lang=localStorage.getItem('msfWacaPrep2026Lang')||'en';
  const theme='light';
  body.classList.add('lang-'+lang); root.lang=lang; root.dir=lang==='ar'?'rtl':'ltr'; root.dataset.theme='light'; try{localStorage.removeItem('msfWacaPrep2026Theme')}catch(e){}
  ...
  window.setLang=function(l){
    body.classList.remove('lang-en','lang-ar'); body.classList.add('lang-'+l); root.lang=l; root.dir=l==='ar'?'rtl':'ltr'; localStorage.setItem('msfWacaPrep2026Lang',l);
    document.dispatchEvent(new CustomEvent('msf-language-change',{detail:{lang:l}}));
  };
  window.toggleTheme=function(){root.dataset.theme='light';try{localStorage.removeItem('msfWacaPrep2026Theme')}catch(e){}};
  ...
  if(nav && !nav.querySelector('a[href="/msf-waca-prep-2026/upa-evidence-lab.html"]')){
    const label=document.createElement('div');
    label.className='nav-label';
    label.textContent='EVIDENCE & REAL CASES';
    const links=[
      ['/msf-waca-prep-2026/upa-evidence-lab.html','Evidence Lab','مختبر الأدلة'],
      ['/msf-waca-prep-2026/upa-visual-study.html','Visual Study','المذاكرة البصرية'],
      ['/msf-waca-prep-2026/resources.html','Resources','المصادر المساعدة']
    ];
  ...
  if(page && page!=='index.html') {
    try { localStorage.setItem('msfWacaPrep2026LastPage', page); localStorage.setItem('msfWacaPrep2026LastTitle', document.title||page); localStorage.setItem('msfWacaPrep2026LastSeen', new Date().toISOString()); } catch(e) {}
  }
