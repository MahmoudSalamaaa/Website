(function(){
  ...
  let state = {};
  try { state = JSON.parse(localStorage.getItem('msfWacaPrep2026QuestionState') || '{}') || {}; } catch(e){ state = {}; }
  ...
  function persist(){localStorage.setItem('msfWacaPrep2026QuestionState',JSON.stringify(state));updateStats();}
  ...
  function filteredRows(){
    ...
    let rows=qs.filter(q=>{const hay=[q.question_en,q.question_ar,q.answer_en,q.answer_ar,q.experience_en,q.experience_ar,q.coach_en,q.coach_ar,q.red_flag_en,q.red_flag_ar,q.category_en,q.category_ar,q.study_track,q.importance_reason_en].map(norm).join(' ');const statusOk=!status||(status==='due'?isDue(q.id):getStatus(q.id)===status);return(!s||hay.includes(s))&&(!tier||q.importance_tier===tier)&&(!track||q.study_track===track)&&(!category||q.category_en===category)&&(!source||q.source_family===source)&&statusOk});
