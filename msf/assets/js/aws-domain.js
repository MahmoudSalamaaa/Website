...
(function(){
  ...
  const queue=document.getElementById('smartQueue'); if(queue)queue.innerHTML=rows.map(q=>`<a class="review-item" href="/msf/question-bank.html#q-${q.id}"><span class="rank">#${q.interview_rank}</span><span><b>${q.question_en}</b><small>${q.study_track||q.category_en}</small></span><span>→</span></a>`).join('')||'<div class="callout success"><b>No reviews due.</b> Run a mock or open P1.</div>';
