(function(){
  ...
  body.classList.add('lang-'+lang); root.lang=lang; root.dir=lang==='ar'?'rtl':'ltr'; root.dataset.theme=theme;
  ...
  window.toggleTheme=function(){
    const n=root.dataset.theme==='dark'?'light':'dark'; root.dataset.theme=n; localStorage.setItem('msfTheme',n); syncThemeButtons();
  };
  ...
  if(nav && !nav.querySelector('a[href="/msf/upa-evidence-lab.html"]')){
    const label=document.createElement('div');
    label.className='nav-label';
    label.textContent='EVIDENCE & REAL CASES';
    const links=[
      ['/msf/upa-evidence-lab.html','Evidence Lab','مختبر الأدلة'],
      ['/msf/upa-visual-study.html','Visual Study','المذاكرة البصرية'],
      ['/msf/resources.html','Resources','المصادر المساعدة']
    ];
  ...
  function syncThemeButtons(){
    document.querySelectorAll('[onclick*="toggleTheme"]').forEach(b=>{b.setAttribute('aria-label',root.dataset.theme==='dark'?'Switch to light theme':'Switch to dark theme');b.title=b.getAttribute('aria-label')});
  }
