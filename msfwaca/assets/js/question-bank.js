(function(){
  ...
  const controls = {search:$('search'), tier:$('tier'), track:$('track'), category:$('category'), source:$('source'), status:$('status'), sort:$('sort')};
  let state = {};
  try { state = JSON.parse(localStorage.getItem('msfQuestionState') || '{}') || {}; } catch(e){ state = {}; }
  ...
  function persist(){localStorage.setItem('msfQuestionState',JSON.stringify(state));updateStats();}
