(function(){
  ...
  let state = {};
  try { state = JSON.parse(localStorage.getItem('wacaEvidenceState') || '{}') || {}; } catch(e){}
  ...
  function save(){ localStorage.setItem('wacaEvidenceState', JSON.stringify(state)); }

  function filters(q){
    const s=(els.search.value||'').toLowerCase().trim();
    const hay=[q.question_en,q.question_ar,q.answer_en,q.answer_ar,q.experience_en,q.upa_evidence?.domain_label,q.upa_evidence?.sources?.map(x=>x.name).join(' ')].join(' ').toLowerCase();
  ...
  function proof(q){
    return (q.upa_evidence?.proof_points||[]).map(x=>`<li>${esc(x)}</li>`).join('');
  }
  ...
  function card(q){
    ...
    return `<article class="e-card grade-${g.toLowerCase()}" id="q-${id}">
      ...
      <div class="e-body">
        <div class="boundary"><b>Evidence boundary</b><p>${esc(q.evidence_scope_en||'')}</p></div>
        ...
        <section class="experience">
          <div class="section-title"><span>VERIFIED EVIDENCE & SOURCES</span><b>${esc(g)}</b></div>
          <p>${esc(q.experience_en||'')}</p>
          <ul>${proof(q)}</ul>
          <div class="sources">${sourceBadges(q)}</div>
        </section>
        ...
        <section class="arabic" lang="ar" dir="rtl">
          ...
          <h4>اربطها بخبرتك</h4><p>${esc(q.experience_ar||'')}</p>
        </section>

        <div class="coach-grid">
          <div><b>Research anchor</b><p>${esc(q.research_anchor_en||'')}</p></div>
          <div><b>How to deliver</b><p>${esc(q.coach_en||'')}</p></div>
          <div class="danger"><b>Do not overclaim</b><p>${esc(q.red_flag_en||'')}</p></div>
        </div>
